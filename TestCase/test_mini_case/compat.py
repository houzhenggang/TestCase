    ...
    if install:
        ...
        for line in lines:
            if line.startswith('// CRASH: {0}'.format(package)):
                ...
                break
        time.sleep(3)
        lines = os.popen('adb -s {0} shell pm uninstall {1}'.format(adb.sno, package)).readlines()
        os.popen('adb -s {0} wait-for-device'.format(adb.sno)).readlines()
...
def execute(workout):
    ...
    for filename in glob.glob(os.path.join(unicode(remotedir, 'utf-8'), '*.apk')):
        os.popen('adb -s {0} push \"{1}\" /data/local/tmp/tmp.apk'.format(adb.sno, filename.encode('gb2312'))).readlines()
        r1 = install()
        r2 = install(True)
